...
vec4 lowCol=vec4(0.5,0.1,0.1,1);
...
void main()
{
    ...
    float dotProd=pow(1-dot(dir,vec3(0,1,0)),24);
    vec4 upper = vec4(upCol*(1.0f-dotProd));
    vec4 lower = vec4(lowCol*(dotProd));

    gl_FragDepth=0.99999;
    
    FragColor=mix(upper,lower,dotProd);
    vec3 color = atmosphere(
        normalize(dir),           // normalized ray direction
        vec3(0,6372e3,0),               // ray origin
        sunPose,                        // position of the sun
